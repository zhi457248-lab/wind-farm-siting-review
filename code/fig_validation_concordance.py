"""
Worked illustration of the WFSSF Tier 1-4 pipeline on six indicator profiles.

Tier 1 quantities are derived from the REAL SCADA archive via
fig7_combined.py (load_site / compute_site_stats) and are gated with
absolute IEC 61400-1 commercial-viability thresholds.

Tier 2-4 quantities in GIS_PROXY below are CONSTRUCTED ANALYST SCORES on
[0,1]. They were NOT measured or collected for these sites. They exist only
to exercise the logic of the sequential gate and are held fixed across all
analyses reported in the manuscript.

IMPORTANT: no concordance or accuracy statistic is reported in the paper.
Because the Tier 2-4 scores are analyst-specified, comparing framework
verdicts against an "expected" verdict would be circular by construction.
This script therefore reports gate verdicts only.

Outputs:
  - tab_validation_concordance.csv  (per-profile tier status)
  - fig_validation_concordance.png/pdf  (gate pass/fail matrix)
"""
import numpy as np
import pandas as pd
...
print('=' * 70)
print('WFSSF Tier 1-4 gate verdicts (worked illustration)')
print('=' * 70)
print(results_df.to_string(index=False))

# ---- No concordance statistic -------------------------------------------------
# Tier 2-4 scores are analyst-specified, so comparing verdicts against an
# "expected" verdict would be circular. Verdicts are reported; agreement
# is deliberately not scored.
NOTE = ('Tier 2-4 inputs are constructed analyst scores, not measured site '
        'attributes; gate verdicts are illustrative and no concordance or '
        'accuracy statistic is computed or reported.')
print()
print(NOTE)
...
for i, v in enumerate(results_df['verdict']):
    label = 'Pass all tiers' if v == 'Pass all tiers' else v
    col = '#1B5E20' if v == 'Pass all tiers' else '#B71C1C'
    ax.text(len(tier_cols) + 0.15, i, label, ha='left', va='center',
            fontsize=9.5, fontweight='bold', color=col)

# Summary line above the matrix
summary_y = len(sites) + 0.78
summary_text = 'Illustrative gate verdicts (no accuracy statistic reported)'
